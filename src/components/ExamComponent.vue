<template>
  ...
  export default {
    ...
    watch: {
        ...
        handler(newValue) {
            if(newValue) {
              console.log(this.audioListeningCount, this.listeningToAudioCount)
    ...
    methods: {
      ...
      //   const examType = urlParams.get('exam_type');
      //   try {
      //     const exam = await axios.get(`${this.baseUrl}/exams/get-exam/${examType}/${studentId}`);
      //     this.questionNumber = exam.data.data?.question_number
      //     this.totalQuestions = exam.data.data?.question_count
      //     this.isAudioLevel = exam.data.data?.is_audio_level
      //     this.listeningToAudio = exam.data.data?.listening_to_audio
      //     this.listeningToAudioCount = exam.data.data?.listening_to_audio_count
      //     this.audioListeningCount = exam.data.data?.audio_listening_count
      //   } catch (error) {
      ...
      async fetchNextQuestion() {
        if(this.isAudioLevel && this.listeningToAudio) {
          this.checkAudioLevel();
        } else {
          console.log(this.questionNumber < this.totalQuestions && !this.examCompleted)
